...
class PEKSClient:
    ...
    # For a public key A_pub and a word W, produces a searchable
    # encryption of W.
    @log_errors
    def PEKS(self, W):
        r = self._curve.randomElement()
        t = encrypt(self.H1(W), self._curve.x25519(self.h, r))
...
class PEKSServer:
    ...
    # Given a searchable encryption S = PEKS(A_pub, W') and a Trapdoor
    # T_w = Trapdoor(A_priv, W), output 'True' if W = W' and 'False' otherwise.
    @log_errors
    def Test(self, S, T_w):
        A, B = S
        A = int_to_bytes(A)
